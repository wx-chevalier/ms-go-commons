...
func TestUnZip(t *testing.T) {
	t.Log("unzip zipped file")
	{
		...
		content, err := ReadStringFromFile(filepath.Join(tmpDir, "sourceFile"))
		require.NoError(t, err)
	...
	t.Log("unzip zipped dir")
	{
		...
		isDir, err := IsDirExists(filepath.Join(tmpDir, "sourceDir"))
		require.NoError(t, err)
	...
	t.Log("unzip into different dir")
	{
		...
		sourceTmpDir, err := NormalizedOSTempDirPath("__1__")
		require.NoError(t, err)
		...
		destTmpDir, err := NormalizedOSTempDirPath("__2__")
		require.NoError(t, err)

		require.NoError(t, UnZip(destinationZip, destTmpDir))
		exist, err := IsPathExists(filepath.Join(destTmpDir, "sourceFile"))
		require.NoError(t, err)
		require.Equal(t, true, exist)
		// ---
	}
	...
	t.Log("relative path")
	{
		...
		revokeFn, err := RevokableChangeDir(sourceTmpDir)
		require.NoError(t, err)
		defer func() {
		...
		exist, err := IsPathExists("./unzipped/sourceFile")
		require.NoError(t, err)
		require.Equal(t, true, exist)
		// ---
