...
func RetrieveArchive(fs afero.Fs, uri string) (io.ReadCloser, error) {
	...
	if strings.HasPrefix(uri, "http://") || strings.HasPrefix(uri, "https://") {
		...
		if resp.StatusCode != http.StatusOK {
			return nil, errutils.Errorf("http GET returned status %d", resp.StatusCode)
		}
		...
		if err != nil {
			return nil, errutils.Wrapf(err, "getting absolute path for %v", uri)
		}

		f, err := fs.Open(path)
		if err != nil {
			return nil, errutils.Wrapf(err, "opening file %v", path)
		}
		file = f
	}
	return file, nil
