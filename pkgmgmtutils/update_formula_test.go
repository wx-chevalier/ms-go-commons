...
var _ = Describe("package management utils", func() {
	...
	/*	Context("using full GitHub APIs", func() {
		It("glooctl can get latest release version", func() {
			...
			fOpts := []FormulaOptions{
				{
					...
					RepoOwner:      "wx-chevalier",           // Make change in this repo
					RepoName:       "test-homebrew-tap", // Assumes this repo is forked from PRRepo if PRRepoOwner != RepoOwner
					PRRepoOwner:    "wx-chevalier",           // Make PR to this repo
					PRRepoName:     "test-homebrew-tap",
					PRBranch:       "master",
					PRDescription:  "/wip",
					PRCommitName:   "wx-chevalier Bot",
				...
				},
				{
					...
					RepoOwner:       "wx-chevalier",
					RepoName:        "test-fish-food",
					PRRepoOwner:     "wx-chevalier",
			...
			status, err := UpdateFormulas("wx-chevalier", "gloo", dirTmp, `glooctl-(darwin|linux|windows).*\.sha256`, fOpts)
		...
		It("squashctl can get latest release version", func() {
			...
			status, err := UpdateFormulas("wx-chevalier", "gloo", dirTmp, `squashctl-(darwin|linux|windows).*\.sha256`, fOpts)
			Expect(err).NotTo(HaveOccurred())
			if err != nil {
				fmt.Println(err)
			}
		...
		It("supergloo can get latest release version", func() {
			...
			status, err := UpdateFormulas("wx-chevalier", "gloo", dirTmp, `supergloo-cli-(darwin|linux|windows).*\.sha256`, fOpts)
			Expect(err).NotTo(HaveOccurred())
			if err != nil {
				fmt.Println(err)
			}
  ...
  if OS.mac?
    url "https://github.com/wx-chevalier/gloo/releases/download/v#{version}/glooctl-darwin-amd64"
    sha256 "5b741927c40f4a430bcf42905901e204e955a5a5a3a3bbb9e67680df3a188f99"
  elsif OS.linux?
    url "https://github.com/wx-chevalier/gloo/releases/download/v#{version}/glooctl-linux-amd64"
  ...
  desc "Envoy-Powered API Gateway"
  homepage "https://gloo.solo.io"
  url "https://github.com/wx-chevalier/gloo.git",
      :tag      => "v0.13.14",
      :revision => "f2fdf602bd6c96b0ebbc0d75bc8ac48c66f83db9"
  head "https://github.com/wx-chevalier/gloo.git"
  ...
  def install
    ...
    dir = buildpath/"src/github.com/wx-chevalier/gloo"
    ...
    packages = {
        {
            os = "darwin",
            arch = "amd64",
            url = "https://github.com/wx-chevalier/gloo/releases/download/v" .. version .. "/" .. name .. "-darwin-amd64",
        ...
        },
        {
            os = "linux",
            arch = "amd64",
            url = "https://github.com/wx-chevalier/gloo/releases/download/v" .. version .. "/" .. name .. "-linux-amd64",
            -- shasum of the release archive
        ...
        },
        {
            os = "windows",
            arch = "amd64",
            url = "https://github.com/wx-chevalier/gloo/releases/download/v" .. version .. "/" .. name .. "-windows-amd64.exe",
            -- shasum of the release archive
